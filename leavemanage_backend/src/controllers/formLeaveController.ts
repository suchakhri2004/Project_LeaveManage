...
export const leaveRequest = async (req: RequestWithToken, res: Response) => {
  try {
    ...
    if (checkPosition.rows[0].positionname === "รองปลัดเทศบาลระดับต้น" || checkPosition.rows[0].positionname === "รองปลัดเทศบาลระดับกลาง") {
      const query = `
        INSERT INTO leaverequest (
...
export const opinionsOnly = async (req: RequestWithToken, res: Response) => {
  ...
  try {
    ...
    } else if (checkPosition.rows[0].positionname === "รองปลัดเทศบาลระดับต้น" || checkPosition.rows[0].positionname === "รองปลัดเทศบาลระดับกลาง") {
      await pool.query(
